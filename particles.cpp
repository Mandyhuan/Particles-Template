...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Huan Yan" };
...
const char *CL_FILE_NAME   = { "yanhua.cl" };
...
char *GetVenderNameFromID(unsigned int id)
{
...
void PrintDeviceExtensions(cl_device_id device)
{
    char extensions[1024];
    int length;

    memset(extensions, 0, 1024);
...
InitCL( )
{
    ...
    cl_uint idx = 0;
    cl_uint NumDevices;
    for (idx = 0; idx < NumPlatforms; idx++)
    {
        status = clGetDeviceIDs(Platforms[idx], CL_DEVICE_TYPE_GPU, 1, &Device, &NumDevices);
    ...
    if (!bFoundOne)
    {
        cout<<"Can not found a device with GPU!"<<endl;
    }
    else
    {
	    char strBuf[512];
        ...
        cout <<"Platform #"<<idx<<endl;
 
        memset(strBuf, 0, 512);
		clGetPlatformInfo(Platform, CL_PLATFORM_NAME, 512, strBuf, NULL);
		cout<<"       Name:    =\'"<<strBuf<<"\'"<<endl;

		memset(strBuf, 0, 512);
		clGetPlatformInfo(Platform, CL_PLATFORM_VENDOR, 512, strBuf, NULL);
		cout<<"       Vendor:  =\'"<<strBuf<<"\'"<<endl;

        memset(strBuf, 0, 512);
		clGetPlatformInfo(Platform, CL_PLATFORM_VERSION, 512, strBuf, NULL);
		cout<<"       Version: =\'"<<strBuf<<"\'"<<endl;

        memset(strBuf, 0, 512);
		clGetPlatformInfo(Platform, CL_PLATFORM_PROFILE, 512, strBuf, NULL);
		cout<<"       Profile: =\'"<<strBuf<<"\'"<<endl;
        ...
        for (unsigned int dev = 0; dev < NumDevices; dev++)
        {
            ...
            cout<<"               Type = 0x"<<hex<<setfill('0')<<setw(4)<<CL_DEVICE_TYPE_GPU<<" = CL_DEVICE_TYPE_GPU"<<endl;

		    clGetDeviceInfo(Device, CL_DEVICE_VENDOR_ID, sizeof(cl_uint), (void *)&uint_entry, NULL);
		    cout<<"               Device Vender ID = 0x"<<uint_entry<<"("<<GetVenderNameFromID(uint_entry)<<")"<<endl;

            clGetDeviceInfo(Device, CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(cl_uint), &uint_entry, NULL);
		    cout<<"               Device Maximum Compute Uints = "<<dec<<uint_entry<<endl;
            
            clGetDeviceInfo(Device, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(cl_uint), &uint_entry, NULL);
		    cout<<"               Device Maximum Work Item Dimensions = "<<uint_entry<<endl; 
            
            size_t work_item_size[3];
            clGetDeviceInfo(Device, CL_DEVICE_MAX_WORK_ITEM_SIZES, 3 * sizeof(cl_uint), work_item_size, NULL);
		    cout<<"               Device Maximum Work Item Sizes = "<<work_item_size[0]<<" x "<<work_item_size[1]<<" x "<<work_item_size[2]<<endl;

            clGetDeviceInfo(Device, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(cl_uint), &uint_entry, NULL);
		    cout<<"               Device Maximum Work Group Size = "<<uint_entry<<endl;

            clGetDeviceInfo(Device, CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(cl_uint), &uint_entry, NULL);
		    cout<<"               Device Maximum Clock Frequency = "<<uint_entry<<" MHz"<<endl;
